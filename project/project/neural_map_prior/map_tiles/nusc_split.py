...
if __name__ == '__main__':
    ...
    split_scenes_train = filter_scenes(nusc, TRAIN_SCENES, in_boston=False)
    split_scenes_val = filter_scenes(nusc, VAL_SCENES, in_boston=False)

    sample_tokens_train = [get_sample_token(nusc, scene) for scene in split_scenes_train]
    sample_tokens_train = reduce(lambda x, y: x + y, sample_tokens_train)

    sample_tokens_val = [get_sample_token(nusc, scene) for scene in split_scenes_val]
    sample_tokens_val = reduce(lambda x, y: x + y, sample_tokens_val)
    print(len(sample_tokens_train), len(sample_tokens_val))
