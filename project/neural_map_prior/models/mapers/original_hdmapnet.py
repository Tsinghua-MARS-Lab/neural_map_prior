...
class NeuralMapPrior(BaseMapper):
    ...
    def __init__(self,
                 ...
                 map_attribute=None,
                 open_nmp=False,
                 positional_encoding_cur=None,
                 positional_encoding_prior=None,
                 **kwargs):
        super(NeuralMapPrior, self).__init__()
        ...
        if self.open_nmp:
            ...
            self.positional_encoding_cur = build_positional_encoding(positional_encoding_cur)
            self.positional_encoding_prior = build_positional_encoding(positional_encoding_prior)
    ...
    def get_bev_pos(self, batch_size, dtype, device):
        bev_mask = torch.zeros((batch_size, self.bev_h, self.bev_w)).to(device)
        cur_pos = self.positional_encoding_cur(bev_mask).to(dtype)

        bev_mask = torch.zeros((batch_size, self.bev_h, self.bev_w)).to(device)
        prior_pos = self.positional_encoding_prior(bev_mask).to(dtype)
        return cur_pos, prior_pos
    ...
    def forward_single(self, imgs, **kwargs):
        ...
        if self.open_nmp:
            ...
            cur_pos, prior_pos = self.get_bev_pos(batch_size, imgs.dtype, imgs.device)
            ...
            cur_bev = cur_bev + cur_pos
            prior_bev = prior_bev + prior_pos
